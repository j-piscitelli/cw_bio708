...
print(s_mu20 < s_mu5) # returns TRUE
...
pnorm(q = 10, mean = 5, sd = 3) - pnorm(q = 0, mean = 5, sd =3) # I think "qnorm()" above should be "pnorm()"

# 2. Calculate the probability that 0 < x <= 10 for a normal distribution with mean = -5 and sd = 3.
pnorm(q = 10, mean = -5, sd = 3) - pnorm(q = 0, mean = -5, sd =3)
...
pl_seto <- df_iris %>% 
  ...
  pull(Petal.Length) # vector of setosa petal lengths
pl_vers <- df_iris %>% 
  filter(Species == "versicolor") %>% 
  pull(Petal.Length) # vector of versicolor petal lengths
# Checking the standard deviations
sd(pl_seto)
sd(pl_vers)
# They are pretty different, considering the range of petal lengths
# so var.equal = FALSE
t.test(x = pl_seto,
       y= pl_vers,
       var.equal = FALSE)
...
summary(aov(data = df_insect,
            count ~ spray)) # Pr(>F) is <2e-16, indicating a highly significant difference
